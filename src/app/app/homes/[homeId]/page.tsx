...
import { HomeAdminInviteForm } from "@/components/homes/home-admin-invite-form";
...
export default async function HomeAdminPage({ params }: HomeAdminPageProps) {
  ...
  let members;
  try {
    members = await getHomeMembers(homeId);
  } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-pretty text-sm text-[var(--neu-text-muted)]">{home.name}</p>
      </div>
      ...
      <NeuSurface variant="raised" className="p-5 sm:p-6">
        ...
        <div className="mt-4">
          <HomeAdminInviteForm homeId={homeId} />
        </div>
      </NeuSurface>
    </div>
  );
