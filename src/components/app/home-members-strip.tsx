...
function roleLabel(role: BackendHomeMemberDto["role"]): string {
  return role === "owner" ? "Admin" : "Miembro";
}

type HomeMembersStripProps = {
  members: BackendHomeMemberDto[] | null;
  loading: boolean;
  error: string | null;
};

export function HomeMembersStrip({ members, loading, error }: HomeMembersStripProps) {
  return (
