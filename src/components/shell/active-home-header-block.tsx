...
export function ActiveHomeHeaderBlock() {
  const router = useRouter();
  const { homes, selectedHome, setSelectedHomeId } = useSelectedHome();
  const [open, setOpen] = useState(false);
  const wrapRef = useRef<HTMLDivElement>(null);
  const menuId = useId();

  const text = selectedHome?.name ?? "Sin hogar activo";
  const showSwitcher = homes.length > 1;
          ...
          {open ? (
            ...
            >
              <ul className="flex flex-col gap-0.5 p-1">
                {homes.map((h) => {
                  const isSel = h.id === selectedHome?.id;
                  return (
                    <li key={h.id}>
                      <button
                        type="button"
