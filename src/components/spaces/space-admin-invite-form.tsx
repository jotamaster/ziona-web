...
type SpaceAdminInviteFormProps = {
  spaceId: string;
};

export function SpaceAdminInviteForm({ spaceId }: SpaceAdminInviteFormProps) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await createInvitationAction(spaceId, publicCode);
      if (result.ok) {
        setPublicCode("");
        router.refresh();
        return;
