...
import { writeSelectedSpaceIdToStorage } from "@/lib/spaces/resolve-selected-space";
...
export function CreateSpaceDialog({ open, onOpenChange }: CreateSpaceDialogProps) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result: CreateSpaceState = await createSpaceAction(name);
      if (result.ok) {
        writeSelectedSpaceIdToStorage(result.spaceId);
        close();
        router.refresh();
        return;
      }
